...
- [x] Choose smallest sound schema; bounded physical size estimate before implementation.
- [x] Staged migration, deterministic ETL integration and compatibility reads.
- [x] Actual source data / broad fixture complete parity, integrity, idempotency and failure proof.
- [x] Realistic PostgreSQL storage and query benchmarks.
- [x] Blue/green plan including all governed and service state, rollback and authorization gates.
- [x] Relevant local tests, exact-head hosted CI, final diff review and draft PR.
...
Implementation and review packet completed in draft PR #188. Exact-head hosted validation remains the final review gate; results are recorded in that PR. Local Docker could not run, so actual PostgreSQL proofs use hosted disposable containers. No production execution is authorized.
...
## Hosted correction
3745c95 changed the frozen historical receipt job service from PostgreSQL 16 to 17, triggering the existing M14E/M14F/M14G job-definition preservation assertions. Restore that historical definition byte-for-byte. PostgreSQL 17 now runs in an owned, bounded container only inside the appended normalization step, on loopback port 55432, stopped by an EXIT trap. No historical assertion is weakened and no permanent new CI job is added.

## Final reconciliation
The bounded implementation, actual-data capture, parity/size/performance proof, active ETL adapters and blue/green migration/rollback review package are complete. Current-head CI is the final gate recorded in PR #188; completed predecessor runs establish full-data and API parity plus compact restore. The remaining execution prerequisites are a reviewed green target/project slot, write freeze, fresh export and target-specific validation, configuration/cutover approval and rollback retention. The missing historical Senate XML caches are separately reported; they are not replaced or used to weaken tests. M15B remains paused. Production writes performed: no.
