...
Senate amendment fact-model preflight is dry-run only. `app.etl.senate_amendment_facts` reads cached 119th Congress / 2025 Senate XML and builds local review manifests that preserve amendment number, parent bill, amendment purpose, roll question/title, source URL, and member vote-row counts. These manifests are not import manifests and are not approval artifacts. Amendment rows must not be imported through the bill-centered fact path because storing only the parent `bill_id` would collapse amendment identity into generic parent-bill evidence. A future amendment fact import requires an explicit schema/model decision, likely a dedicated amendment reference table keyed to `roll_calls`, plus UI/API handling that labels amendment rows as fact-only and non-interpreted until separate review.
