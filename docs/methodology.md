...
## Roll-Call Identity and Current-Congress Refreshes

Roll-call identity is session-aware. The canonical roll-call key is:
...
The canonical amendment evidence path is documented in `docs/amendment_evidence_pipeline.md` and implemented by the shared contracts in `app.etl.amendment_evidence`. New amendment-family work should reuse the existing fetch/cache, identity normalization, source-packet, supervised/manual validation, bounded-write, rollback, recompute, and idempotency contracts before adding a permanent ETL module. Amendment identity, parent-measure relationships, chamber, Congress, and session must remain explicit; parent-measure context must not replace direct amendment meaning.
