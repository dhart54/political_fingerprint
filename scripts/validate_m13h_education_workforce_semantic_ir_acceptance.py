"""Independently validate M13H accepted Education & Workforce Semantic IR."""
...
NON_DIRECTIONAL_EPISODE_ID = "single-119-hr-1005-1-312"
...
def validate() -> dict[str, Any]:
    ...
    require(
        decision_counts == {"accept_candidate_as_written": 2},
        "M13H decision accounting differs",
    )
    ...
    require(
        ...
        == "mixed",
        "accepted proposition type or direction differs",
    )
    ...
    require(
        ...
        "H.R. 1005 non-directional disposition differs",
    )
    ...
    require(
        ...
        and "house:119:1:312" not in evidence_action_ids,
        "non-directional evidence entered a directional proposition",
    )
    notable = candidate_props[
        "notable-hr1048-amendment-support-final-passage-opposition"
    ]
    require(
        notable["evidence_episode_ids"] == ["hr-1048-amendment-and-final-passage"]
        and notable["evidence_action_ids"] == ["house:119:1:79", "house:119:1:83"]
        and any("whole-package" in value for value in notable["material_limitations"])
        and any(
            "not a trajectory" in value for value in notable["material_limitations"]
        ),
        "H.R. 1048 mixed notable boundary differs",
    )
    require(
        final
        == {
    ...
    require(
        binding["accepted_pr"] == ACCEPTED_M13G_PR
        and binding["accepted_head"] == ACCEPTED_M13G_HEAD
        and binding["post_merge_main"] == POST_M13G_MERGE_MAIN
        and binding["candidate_subject_sha256"] == CANDIDATE_SUBJECT_SHA256,
        "accepted PR #167 binding differs",
    )
    require(
        ...
        "accepted M13G decision template was not empty and non-authorizing",
    )
    require(
    ...
    state = current_state["active_m13h_behavioral_semantic_ir_decision_milestone"]
    require(
        state["post_m13g_merge_main"] == POST_M13G_MERGE_MAIN
        and state["decision_accounting"] == {"accept_candidate_as_written": 2}
        ...
        "M13H current-state identity or authority boundary differs",
    )
    verify_seal(parity, "parity_subject_sha256", "M13H parity")
    require(
