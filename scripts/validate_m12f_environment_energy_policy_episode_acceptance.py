"""Independently validate exact M12F policy-episode acceptance."""
...
from backend.scripts.build_m12f_environment_energy_policy_episode_acceptance import (  # noqa: E402
    AUTHORITY_PATH,
...
def validate() -> dict[str, Any]:
    ...
    require(
        ...
        "M12F decision coverage differs",
    )
    require(
        all(
        ...
        "M12F authority decision differs from accepted review",
    )
    require(
        authority["subject"]["decision_accounting"]
        == {"accept_candidate_as_written": 63},
        "M12F decision counts differ",
    )
    ...
    for episode_id, source in candidate_by_id.items():
        ...
        require(
            ...
            f"{episode_id}: candidate changed during M12F implementation",
        )
    hr6387 = next(
        row
    ...
    require(
        hr6387["member_direction"] == "non_directional_not_voting",
        "H.R. 6387 direction differs",
    )
    for action_id in ("house:119:1:25", "house:119:1:330"):
        ...
        require(
            ...
            and episode["grouping_type"] == "single_action",
            f"{action_id}: package boundary differs",
        )
    ...
    m12f = state["active_m12f_policy_episode_decision_milestone"]
    require(
        m12f["milestone_state"] == "completed_mechanical_acceptance_validated"
        and m12f["post_m12e_merge_main"] == "450a759c5a2d0eaf767e68bc999c7d3ec8e9ca1e"
        and m12f["decision_accounting"] == {"accept_candidate_as_written": 63}
        and m12f["accepted_action_count"] == 63
        and m12f["accepted_episode_count"] == 63
        and m12f["single_action_episode_count"] == 63
        and m12f["multi_action_episode_count"] == 0
        and m12f["cross_measure_episode_count"] == 0
        and m12f["blocked_action_count"] == 0
        and m12f["authority"]["sha256"] == file_sha(AUTHORITY_PATH)
        and m12f["implementation"]["sha256"] == file_sha(IMPLEMENTATION_PATH)
        and m12f["canonical_internal_policy_episode_membership"] is True
        and m12f["canonical_semantic_ir_acceptance"] is False
        and not any(m12f["downstream_authorizations"].values()),
        "M12F current-state boundary differs",
    )
    require(
