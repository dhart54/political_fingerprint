"""Independently validate detached M13G behavioral Semantic IR candidates."""
...
from backend.scripts.build_m13g_education_workforce_behavioral_semantic_ir_candidates import (  # noqa: E402
    AUTHORITY_PATH,
    DECISION_PATH,
...
def validate() -> dict[str, Any]:
    ...
    require(
        canonical_file_sha256(AUTHORITY_PATH) == M13F_AUTHORITY["file_sha256"],
        "M13F authority changed",
    )
    require(
        authority["authority_subject_sha256"]
        == M13F_AUTHORITY["authority_subject_sha256"],
        "M13F authority subject changed",
    )
    require(
        canonical_file_sha256(IMPLEMENTATION_PATH)
        == M13F_IMPLEMENTATION["file_sha256"],
        "M13F implementation changed",
    )
    require(
        implementation["implementation_subject_sha256"]
        == M13F_IMPLEMENTATION["implementation_subject_sha256"],
        "M13F implementation subject changed",
    )
    ...
    require(
        len(episodes) == len(ledger) == 16 and set(episodes) == set(ledger),
    ...
    require(
        len(accounting) == len(accounting_by_id) == 16
    ...
    non_directional_id = "single-119-hr-1005-1-312"
    require(
        ...
        "H.R. 1005 non-directional accounting differs",
    )
    mixed_episode_id = "hr-1048-amendment-and-final-passage"
    mixed_episode = episodes[mixed_episode_id]
    require(
        mixed_episode["member_direction"] == "mixed_on_episode_choices"
        and mixed_episode["primary_action_ids"] == ["house:119:1:79", "house:119:1:83"]
        and {
            row["action_id"]: row["accepted_exact_choice_position_effect"]
            for row in mixed_episode["actions"]
        }
        == {
            "house:119:1:79": "supports_exact_choice",
            "house:119:1:83": "opposes_exact_choice",
        }
        and accounting_by_id[mixed_episode_id]["primary_proposition_id"]
        == "notable-hr1048-amendment-support-final-passage-opposition",
        "H.R. 1048 mixed episode boundary differs",
    )
    require(not graph["synthesis_propositions"], "synthesis leaked into M13G")
    require(
    ...
    m13g = state["active_m13g_behavioral_semantic_ir_candidate_milestone"]
    downstream = m13g["downstream_authorizations"]
    require(
        m13g["milestone_state"] == "complete_pending_independent_substantive_review"
        and m13g["accepted_episode_count"] == 16
        and m13g["proposed_repeated_pattern_count"] == 1
        and m13g["proposed_trajectory_count"] == 0
        and m13g["proposed_notable_choice_count"] == 1
        and m13g["proposition_episode_count"] == 3
        and m13g["limit_or_contrast_episode_count"] == 1
        and m13g["no_safe_higher_level_proposition_episode_count"] == 11
        and m13g["unused_non_directional_episode_count"] == 1
        and m13g["overlapping_primary_owner_count"] == 0
        and m13g["candidate_identity"]["sha256"] == canonical_file_sha256(GRAPH_PATH)
        and m13g["candidate_identity"]["candidate_subject_sha256"]
        == graph_artifact["candidate_subject_sha256"]
        and m13g["decision_template_identity"]["all_decisions_empty"] is True
        and m13g["semantic_ir_acceptance_state"] == "not_started_not_authorized"
        and not any(downstream.values()),
        "M13G current-state boundary differs",
    )
