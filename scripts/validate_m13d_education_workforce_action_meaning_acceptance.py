...
def validate_repository() -> dict[str, Any]:
    ...
    require(
        ...
        == ACCEPTED_CANDIDATE_SUBJECT_SHA256,
        "accepted M13C identity differs",
    )
    require(
        binding
        == {
    ...
    require(
        len(decisions) == len(records) == len(candidates) == 17
        and set(decisions) == set(records) == set(candidates),
        "exact 17-record action set differs",
    )
    for action_id, decision in decisions.items():
        ...
        require(
            record["record_id"].endswith(":m13d:v1")
    ...
    require(
        decisions["house:119:1:312"]["accepted_exact_choice_position_effect"]
    ...
    m13c = state["active_m13c_action_interpretation_milestone"]
    m13d = state["active_m13d_action_interpretation_decision_milestone"]
    require(
        m13c["reviewed_pr"] == ACCEPTED_PR
        and m13c["reviewed_head"] == ACCEPTED_HEAD
        and m13c["post_merge_main"] == POST_M13C_MERGE_MAIN
        and m13d["accepted_decision_count"] == 17
        and m13d["source_blocked_count"] == 0
        and m13d["authority"]["sha256"] == file_sha256(AUTHORITY_PATH)
        and m13d["authority"]["authority_subject_sha256"]
        == authority["authority_subject_sha256"]
        and m13d["implementation"]["sha256"] == file_sha256(IMPLEMENTATION_PATH)
        and m13d["implementation"]["implementation_subject_sha256"]
        == implementation["implementation_subject_sha256"]
        and m13d["canonical_internal_action_interpretation"] is True
        and m13d["canonical_semantic_acceptance"] is False
        and all(value is False for value in m13d["downstream_authorizations"].values()),
        "M13C/M13D current state differs",
    )
