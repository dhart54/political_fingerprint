...
CREATE TABLE IF NOT EXISTS house_member_service_evidence (
    ...
    service_date_precision TEXT NOT NULL CHECK (service_date_precision IN ('year', 'exact_date')),
    party TEXT,
    official_url TEXT,
    ...
    UNIQUE (bioguide_id, congress, canonical_state, canonical_district, source_name, source_checksum),
...
CREATE TABLE IF NOT EXISTS house_seat_status_evidence (
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (congress, canonical_state, canonical_district, source_name, source_checksum),
    CHECK ((seat_status <> 'vacant') OR (current_legislator_id IS NULL AND current_bioguide_id IS NULL))
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_house_seat_status_one_current_seat
    ON house_seat_status_evidence (congress, canonical_state, canonical_district)
    WHERE metadata_currentness IN ('current_cross_source_confirmed', 'current_primary_source_only', 'vacant_officially_confirmed');
