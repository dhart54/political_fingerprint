...
from scripts.validate_m12h_environment_energy_semantic_ir_acceptance import validate
...
def test_m12h_build_and_independent_validator_pass() -> None:
    assert build(check=True)["accepted_proposition_count"] == 3
    result = validate()
    assert result["status"] == "pass"
    assert result["episode_dispositions"]["unused_non_directional_evidence"] == 1


def test_generic_schemas_accept_historical_m11h_and_m12h() -> None:
...
def test_unused_non_directional_disposition_cannot_be_collapsed() -> None:
    ...
    row = next(
        ...
        if row["episode_id"] == "single-119-hr-6387-2-136"
    )
...
def test_contrast_or_no_safe_episode_cannot_enter_accepted_evidence() -> None:
    ...
    record["evidence_lineage"].append(
        {
            "episode_id": "single-119-hr-6387-2-136",
