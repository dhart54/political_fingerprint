...
def manifest_fixture(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    for spec in specs:
        ...
        artifacts.append({**spec, "actual_filename": spec["filename"], "http_status": 200, "content_type": "application/octet-stream", "size_bytes": path.stat().st_size, "sha256": retrieval.sha256_file(path), "retry_history": [{"attempt": 1, "status": 200, "result": "success"}]})
    manifest = retrieval.build_manifest("batch", artifacts)
...
def test_land_population_top_disagreement():
    ...
    assert analysis.compare_land_population(rows)["counts"]["top_disagrees"] == 1
