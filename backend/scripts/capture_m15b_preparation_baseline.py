...
ROOT = BACKEND.parent
OUT = ROOT/'docs/editorial/publication_replacements/m15b_green_authorization'
...
from scripts.green_production_target import target_identity, GREEN_IDENTITY
from scripts.green_vote_storage import schema_state, digest as schema_digest
from scripts.m15b_persistence_preparation import MERGED
...
def capture(conn, identity):
    ...
    assert conn.execute("SELECT relkind FROM pg_class WHERE oid='public.vote_contexts'::regclass").fetchone()['relkind']=='v'
    assert conn.execute("SHOW server_version").fetchone()['server_version']=='17.6'
    sequences = {r['sequencename']:dict(conn.execute(
        sql.SQL('SELECT last_value,is_called FROM public.{}').format(sql.Identifier(r['sequencename']))).fetchone())
        for r in conn.execute("SELECT sequencename FROM pg_sequences WHERE schemaname='public' ORDER BY 1").fetchall()}
    ...
    for table in ('votes_cast','vote_context_members'):
    ...
    fixture={'tables':tables,'sequences':sequences,'normalized_schema':schema}
    absent={}
    for issue,item in prepared['replacements'].items():
    ...
    report={'captured_at_utc':datetime.now(timezone.utc).isoformat(),'transaction_read_only':True,
        'production_target':GREEN_IDENTITY,'merged_main':MERGED,
        'postgresql_version':conn.execute('SHOW server_version').fetchone()['server_version'],
        'database_bytes':conn.execute('SELECT pg_database_size(current_database()) n').fetchone()['n'],
        'normalized_schema_sha256':schema_digest(schema),'sequences':sequences,
    ...
    assert all(dict(conn.execute(sql.SQL('SELECT last_value,is_called FROM public.{}').format(
        sql.Identifier(name))).fetchone())==state for name,state in sequences.items()), 'sequence changed during capture'
...
def main():
    ...
    assert environment.get('NORMALIZED_VOTE_STORAGE')=='1', 'normalized production environment required'
    ...
    assert report['database_bytes']<400000000, 'green capacity exceeds operating threshold'
    OUT.mkdir(parents=True,exist_ok=True)
    assert not (OUT/'production_baseline.json').exists(), 'preserve existing baseline; use a separately reviewed refresh'
