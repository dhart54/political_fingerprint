...
MERGED='f790163f90b8a71fae2bee3206f73953364f1e72'
...
def build():
    ...
    require(baseline['production_target']==GREEN_IDENTITY, 'green baseline identity differs')
    require(baseline['merged_main']==MERGED, 'green baseline code binding differs')
    products={}
    for issue,item in approved['replacements'].items():
        ...
        body={'registry_key':item['registry_key'],'expected_old':old,'proposed_new':{**new,'artifact_id':None},
            ...
            'production_target':GREEN_IDENTITY,
            'normalized_schema_sha256':baseline['normalized_schema_sha256'],
...
def preflight(conn,bundle):
    ...
    require(schema_digest(schema_state(conn))==bundle['normalized_schema_sha256'], 'normalized schema drift')
    require(conn.execute("SELECT relkind FROM pg_class WHERE oid='public.vote_contexts'::regclass").fetchone()['relkind']=='v', 'normalized compatibility view required')
    require(conn.execute('SELECT pg_database_size(current_database()) n').fetchone()['n']<400000000, 'green capacity exceeds operating threshold')
...
def main():
    ...
    environment=dotenv_values(args.env_path) if args.env_path else os.environ
    if args.target=='production':
        require(environment.get('NORMALIZED_VOTE_STORAGE')=='1', 'normalized production environment required')
    url=environment.get('DATABASE_URL') if args.env_path else os.environ[args.database_url_env]
    ...
    with store._connect(url,autocommit=False) as conn:
        if args.mode=='preflight':
            ...
            result={'transaction_read_only':True,'state':preflight(conn,package['replacements'][args.issue]),
                    'production_target_identity_sha256':identity,'package_sha256':package['package_sha256'],
                    'code_head':args.expected_code_head,'issue':args.issue,'writes':0,
                    'database_bytes':conn.execute('SELECT pg_database_size(current_database()) n').fetchone()['n']}
        else:
            conn.execute("SET LOCAL timezone='UTC'")
            if args.mode=='persist': result=persist(conn,package,args.issue)
