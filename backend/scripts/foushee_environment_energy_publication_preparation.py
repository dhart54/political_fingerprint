"""M12N production-eligibility and publication-activation candidate tooling.
...
from app.editorial_presentations.site_publication import (  # noqa: E402
    ENVIRONMENT_ACTIVATION_AUTHORITY_ID as ACTIVATION_AUTHORITY_ID,
...
CONGRESS = 119
POST_M12M_MAIN = "0f34962526a693625f7b15a96baec95121c2f6c9"
M12M_ACCEPTED_HEAD = "bfcfff5f962dd7cc5911764f1690edbe36564d03"
M12M_ACCEPTED_PR = 156
AUTHORITY_ID = (
    "production-eligibility-publication-authority:f000477:environment_energy:119:v1"
)
WRITE_SET_ID = "publication-activation-write-set:f000477:environment_energy:119:v1"
ACTIVATION_TEMPLATE_ID = (
    "human-publication-activation-decision-template:f000477:environment_energy:119:v1"
)
BUNDLE_ID = "foushee_environment_energy_119_publication_activation_v1"
SOURCE_KEY = f"{M12M_ARTIFACT_ID}:publication-source-manifest"
VALIDATION_KEY = f"{M12M_ARTIFACT_ID}:publication-validation"
...
M12L_AUTHORITY_PATH = M12L_ROOT / "human_public_wording_authority.json"
M12L_IMPLEMENTATION_PATH = M12L_ROOT / "reviewed_wording_decision_implementation.json"
M12L_PARITY_PATH = M12L_ROOT / "implementation_parity_manifest.json"
OUTPUT_ROOT = (
...
CURRENT_STATE_PATH = OUTPUT_ROOT / "current_state.json"
ROLLBACK_CONTRACT_PATH = OUTPUT_ROOT / "rollback_contract.json"
...
RUNTIME_SOURCE_PATHS = (
    ...
    BACKEND / "app/editorial_presentations/environment_integration_candidate.py",
    BACKEND / "scripts/foushee_environment_energy_publication_preparation.py",
)
FROZEN_M12N_REVIEWED_RUNTIME_MANIFEST = {
...
FROZEN_FAILED_ACTIVATION_REVIEWED_RUNTIME_MANIFEST = {
    "schema_version": "m12n_reviewed_runtime_manifest_v1",
    "files": [
        {
            "path": "backend/app/api/positions.py",
        ...
        },
        {
            "path": "backend/app/editorial_presentations/site_publication.py",
            "file_sha256": (
                "302ed792fe3cac56e844328bacf1b59808728d58f83d00d4f5d45040701a7808"
            ),
        },
        {
            "path": (
        ...
        },
        {
            ...
            "file_sha256": (
                "7a6f4d4e069a496e10b976c9d21107371b80996ae9d619e3e4a8e2c8e7b29e0f"
            ),
        },
    ],
    "reviewed_runtime_manifest_sha256": (
...
EXPECTED_M12L = {
    M12L_AUTHORITY_PATH: (
        "5079d25c6e6f83f868925d14e429e9c5039a3c7aeb1a824660acefbb5ae2d9fc"
    ),
    M12L_IMPLEMENTATION_PATH: (
        "59b3003df37f1a7c4b02b30d74161a6482b4b6605b2da6e257d84e0436ee8b64"
    ),
    M12L_PARITY_PATH: (
        "e47c7771fcdcc52b369d341ccaece90a3f1acfd2eae4f71634bd64bbd03b77d6"
    ),
}
JUSTICE_NATURAL_KEY = (
...
NATIONAL_SECURITY_NATURAL_KEY = (
    "site-integration-candidate:f000477:national_security_foreign:119:v1"
)
NATIONAL_SECURITY_CONTENT_SHA256 = (
    "05661086601991075f04195090a41e0febaad7f8e6acda53f0cab838f97e860c"
)

load_site_integration_candidate = load_environment_site_integration_candidate
validate_site_integration_candidate = validate_environment_site_integration_candidate
...
def _write_or_check(path: Path, content: str, *, check: bool) -> None:
    if check:
        if not path.exists() or path.read_text(encoding="utf-8") != content:
            raise StoreSafetyError(f"deterministic M12N artifact differs: {path}")
        return
    _write(path, content)
...
def reviewed_runtime_manifest_for_preflight(
    ...
    frozen = FROZEN_M12N_REVIEWED_RUNTIME_MANIFEST
...
def validate_runtime_health_proof(
    ...
    *,
    ...
    if (
        proof.get("schema_version") != "m12n_live_runtime_health_proof_v1"
...
def _target_rows(conn: Any) -> list[dict[str, Any]]:
    return [
        ...
        for row in conn.execute(
            ...
            ([M12M_ARTIFACT_ID, SOURCE_KEY, VALIDATION_KEY],),
        ).fetchall()
    ]
...
def _selector_state(
    ...
    for scope in ("119", "all", "118"):
        ...
        scopes[scope] = {
            ...
            if item["issue_id"]
            in {"JUSTICE_PUBLIC_SAFETY", "NATIONAL_SECURITY_FOREIGN", ISSUE_ID}
        }
    ...
    *,
    ...
    if len(registry) != 2:
        raise StoreSafetyError(
            "Justice and National Security are not the exact baseline"
        )
    by_issue = {row["issue_id"]: row for row in registry}
    justice = by_issue.get("JUSTICE_PUBLIC_SAFETY")
    national_security = by_issue.get("NATIONAL_SECURITY_FOREIGN")
    if (
        justice is None
        or justice["member_bioguide_id"] != MEMBER_ID
    ...
    if (
        ...
        or national_security["member_bioguide_id"] != MEMBER_ID
        or national_security["natural_key"] != NATIONAL_SECURITY_NATURAL_KEY
        or national_security["content_sha256"] != NATIONAL_SECURITY_CONTENT_SHA256
        or national_security["publicly_active"] is not True
    ):
        raise StoreSafetyError("accepted National Security production identity differs")
    if targets:
        raise StoreSafetyError("M12N natural keys already exist")
    if any(
        selector["scopes"][scope][ISSUE_ID]["tier"] != "receipts_only"
        for scope in ("119", "all", "118")
    ):
        raise StoreSafetyError("Environment & Energy selector is not fail-closed")
    if (
    ...
    if (
        selector["scopes"]["119"]["NATIONAL_SECURITY_FOREIGN"]["tier"]
        != "reviewed_conclusion"
        or selector["scopes"]["all"]["NATIONAL_SECURITY_FOREIGN"]["tier"]
        != "reviewed_conclusion"
        or selector["scopes"]["118"]["NATIONAL_SECURITY_FOREIGN"]["tier"]
        != "receipts_only"
    ):
        raise StoreSafetyError("accepted National Security selector state differs")
    report = {
        "schema_version": "m12n_current_production_preflight_v1",
        "captured_at_utc": datetime.now(timezone.utc).isoformat(),
        ...
        "national_security_registry_row": _registry_identity(national_security),
        "environment_registry_rows": [],
        "m12n_target_rows": targets,
...
def validate_preflight(
    ...
    if claimed != semantic_hash(body):
        raise StoreSafetyError("M12N preflight digest mismatch")
    if (
        report.get("schema_version") != "m12n_current_production_preflight_v1"
        or report.get("transaction_read_only") is not True
        or report.get("counts") != CURRENT_COUNTS
        or report.get("m12n_target_rows") != []
        or not SHA40.fullmatch(report.get("deployed_commit", ""))
        or not SHA256.fullmatch(report.get("state_fingerprint_sha256", ""))
    ):
        raise StoreSafetyError("M12N preflight contract differs")
    ...
    if (
        ...
        raise StoreSafetyError("M12N preflight Justice binding differs")
    national_security = report.get("national_security_registry_row", {})
    if (
        national_security.get("natural_key") != NATIONAL_SECURITY_NATURAL_KEY
        or national_security.get("content_sha256") != NATIONAL_SECURITY_CONTENT_SHA256
        or national_security.get("issue_id") != "NATIONAL_SECURITY_FOREIGN"
        or national_security.get("publicly_active") is not True
        or report.get("environment_registry_rows") != []
    ):
        raise StoreSafetyError("M12N preflight publication baseline differs")
    ...
    if runtime_binding is not None and (
        ...
        raise StoreSafetyError("M12N preflight live-runtime binding differs")
    if (
        require_current_runtime
        ...
        raise StoreSafetyError("M12N preflight current-runtime binding differs")
    ...
    if (
        ...
        raise StoreSafetyError("M12N preflight target binding differs")


def build_authority(preflight: dict[str, Any]) -> dict[str, Any]:
    validate_preflight(preflight)
    candidate = load_site_integration_candidate(M12M_PATH)
    subject = {
        "decision": (
            "approve_production_eligibility_and_publication_preparation_candidate"
        ),
        "decision_recorded_at_utc": "2026-08-16T22:36:45Z",
        "reviewer": "dhart54",
        ...
        "accepted_site_integration_binding": {
            ...
            "subject_sha256": M12M_SUBJECT_SHA256,
            "file_sha256": M12M_FILE_SHA256,
            "content_sha256": semantic_hash(candidate),
        },
        "repository_binding": {
            "accepted_m12m_pr": M12M_ACCEPTED_PR,
            "accepted_m12m_head": M12M_ACCEPTED_HEAD,
            "post_m12m_main": POST_M12M_MAIN,
        },
...
def _artifact(
    *,
    ...
    return {
        ...
        "source_commit_sha": POST_M12M_MAIN,
        "member_bioguide_id": MEMBER_ID,
        "issue_id": ISSUE_ID,
        "congress": CONGRESS,
        "chamber": "house",
...
def build_write_set(
    ...
    candidate = load_site_integration_candidate(M12M_PATH)
    validate_publication_authority(authority, candidate=candidate)
    for path, expected in EXPECTED_M12L.items():
        if canonical_file_sha256(path) != expected:
            raise StoreSafetyError(f"accepted M12L file digest differs: {path.name}")
    source_files = [
        _file_record(path)
        for path in (
            M12M_PATH,
    ...
    source_payload = {
        ...
        "accepted_site_integration_subject_sha256": M12M_SUBJECT_SHA256,
        "accepted_site_integration_file_sha256": M12M_FILE_SHA256,
    }
    validation_payload = {
        ...
        "approved_universe_actions": 63,
        "accepted_interpreted_actions": 63,
        "unused_non_directional_actions": [NON_DIRECTIONAL_ACTION_ID],
        "accepted_public_wording_items": 5,
        "accepted_site_integration_subject_sha256": M12M_SUBJECT_SHA256,
        "accepted_site_integration_file_sha256": M12M_FILE_SHA256,
    ...
    relationships = sorted(
        [
            {
                "parent_natural_key": M12M_ARTIFACT_ID,
    ...
    rollback_contract = {
        ...
        "delete_batch_key": f"{BUNDLE_ID}-{POST_M12M_MAIN[:8]}",
        ...
        "national_security_registry_row_unchanged": preflight[
    ...
    metadata = {
        ...
        "active_artifact_sha256": by_key[M12M_ARTIFACT_ID]["content_sha256"],
    ...
    body = {
        ...
        "bundle_id": BUNDLE_ID,
        "deterministic_batch_key": f"{BUNDLE_ID}-{POST_M12M_MAIN[:8]}",
        "source_commit_sha": POST_M12M_MAIN,
...
def validate_write_set(write_set: dict[str, Any], *, authority: dict[str, Any]) -> None:
    ...
    if claimed != semantic_hash(body):
        raise StoreSafetyError("M12N write-set digest mismatch")
    if (
        ...
        or write_set.get("source_commit_sha") != POST_M12M_MAIN
        ...
        raise StoreSafetyError("M12N write-set identity or authority differs")
    if write_set.get("write_caps") != {
        ...
        raise StoreSafetyError("M12N write caps differ")
    if len(write_set["artifacts"]) != 3 or len(write_set["relationships"]) != 2:
        raise StoreSafetyError("M12N activation graph size differs")
    by_key = {item["natural_key"]: item for item in write_set["artifacts"]}
    if set(by_key) != {M12M_ARTIFACT_ID, SOURCE_KEY, VALIDATION_KEY}:
        raise StoreSafetyError("M12N activation natural keys differ")
    if by_key[M12M_ARTIFACT_ID]["payload"] != candidate:
        raise StoreSafetyError("M12N rewrote the accepted M12M artifact")
    for artifact in write_set["artifacts"]:
        if (
            ...
            raise StoreSafetyError("M12N artifact persistence gate differs")
    validation = by_key[VALIDATION_KEY]["payload"]
    if (
        validation["unused_non_directional_actions"] != [NON_DIRECTIONAL_ACTION_ID]
        or validation["production_activation_authorized"] is not False
        or validation["accepted_interpreted_actions"] != 63
        or validation["approved_universe_actions"] != 63
    ):
        raise StoreSafetyError("M12N H.R. 6387 or accounting boundary differs")
    ...
    if (
        ...
        raise StoreSafetyError("M12N rollback does not isolate Justice")
    if (
        ...
        raise StoreSafetyError("M12N rollback does not isolate National Security")
    metadata = write_set["publication_registry"]["publication_metadata"]
    if (
        ...
        raise StoreSafetyError("M12N activation-authority contract differs")
...
def validate_activation_decision_template(
    ...
    if (
        ...
        != "m12n_publication_activation_decision_template_v1"
        ...
        raise StoreSafetyError("M12N activation decision template differs")
...
def _review_packet(
    ...
    packet = {
        "schema_version": "m12n_review_packet_v1",
        "milestone": "M12N",
        "exact_base": POST_M12M_MAIN,
        "accepted_site_integration_binding": authority["subject"][
            "accepted_site_integration_binding"
        ],
        ...
        "national_security_isolation": {
            "natural_key": NATIONAL_SECURITY_NATURAL_KEY,
            "content_sha256": NATIONAL_SECURITY_CONTENT_SHA256,
            "rows_touched": 0,
        },
        "unused_non_directional_action": {
            "action_id": NON_DIRECTIONAL_ACTION_ID,
            "state": "non_directional_not_voting_outside_public_findings",
        },
        ...
        "future_activation_sequence": [
            "merge the mechanically accepted M12N runtime",
            "deploy that exact runtime while the registry target remains absent",
            "verify Environment & Energy remains receipts-only",
...
def _review_markdown(packet: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# M12N Environment & Energy Production-Eligibility Review",
            "",
            ...
            f"- Accepted M12M artifact: `{packet['accepted_site_integration_binding']['artifact_id']}`",
            f"- M12M subject: `{packet['accepted_site_integration_binding']['subject_sha256']}`",
            ...
            "- No updates, no activation-time deletes, and zero Justice or National Security rows touched.",
            "- Rollback deletes only the exact Environment & Energy registry row and its new immutable batch graph.",
            "",
            "## Current public boundary",
            "",
            "- Justice remains active and unchanged.",
            "- National Security remains active and unchanged.",
            "- Environment & Energy remains receipts-only before activation.",
            "- H.R. 6387 remains non-directional Not Voting and excluded from public findings.",
...
def build(
    *, check: bool = False, preflight_path: Path = PREFLIGHT_PATH
    ...
    current_state_subject = {
        "captured_at_utc": preflight["captured_at_utc"],
        "deployed_commit": preflight["deployed_commit"],
        "transaction_read_only": preflight["transaction_read_only"],
        "counts": preflight["counts"],
        "state_fingerprint_sha256": preflight["state_fingerprint_sha256"],
        "active_publication_rows": [
            preflight["justice_registry_row"],
            preflight["national_security_registry_row"],
        ],
        "environment_registry_rows": preflight["environment_registry_rows"],
        "prospective_artifact_rows": preflight["m12n_target_rows"],
        "selector_pre_activation": preflight["selector_pre_activation"],
        "runtime_health_proof_binding": preflight["runtime_health_proof_binding"],
        "production_target_identity_sha256": preflight[
            "production_target_identity_sha256"
        ],
    }
    current_state = {
        "schema_version": "publication_preparation_current_state_v1",
        "artifact_id": "publication-current-state:f000477:environment_energy:119:v1",
        "subject": current_state_subject,
        "current_state_subject_sha256": semantic_hash(current_state_subject),
    }
    rollback_subject = {
        "write_set_binding": activation_write_set_binding(write_set),
    ...
    rollback_contract = {
        ...
        "artifact_id": "publication-rollback-contract:f000477:environment_energy:119:v1",
    ...
    outputs = {
        ...
        CURRENT_STATE_PATH: _json_text(current_state),
        ROLLBACK_CONTRACT_PATH: _json_text(rollback_contract),
    }
    ...
    parity = {
        ...
        "artifact_id": "publication-preparation-parity:f000477:environment_energy:119:v1",
        "subject": parity_subject,
        "parity_subject_sha256": semantic_hash(parity_subject),
    }
    _write_or_check(PARITY_PATH, _json_text(parity), check=check)
    return {
        "authority": authority,
        "write_set": write_set,
...
def _assert_bound_preflight(
    ...
    *,
    ...
    if (
        ...
        raise StoreSafetyError("database state drifted from M12N preflight")
    return actual


def _baseline_fingerprint_without_m12n(conn: Any, *, batch_key: str) -> str:
    keys = [M12M_ARTIFACT_ID, SOURCE_KEY, VALIDATION_KEY]
    queries = (
        (
            """SELECT deterministic_batch_key,source_commit_sha,manifest_sha256,status,
                      artifact_count,relationship_count
...
def _postcheck(
    ...
    *,
    ...
    if _counts(conn) != write_set["expected_counts"]["after"]:
        raise StoreSafetyError("M12N post-activation counts differ")
    batch = conn.execute(
        """SELECT deterministic_batch_key,source_commit_sha,manifest_sha256,status,
                  artifact_count,relationship_count
    ...
    if [_jsonable(dict(row)) for row in batch] != [expected_batch]:
        raise StoreSafetyError("M12N exact batch graph differs")
    artifact_rows = [
        _jsonable(dict(row))
        for row in conn.execute(
    ...
    if artifact_rows != expected_artifacts:
        raise StoreSafetyError("M12N exact artifact graph differs")
    relationship_rows = [
        _jsonable(dict(row))
        for row in conn.execute(
            """SELECT parent.natural_key AS parent_natural_key,
    ...
    if relationship_rows != write_set["relationships"]:
        raise StoreSafetyError("M12N exact relationship graph differs")
    registry = _registry_rows(conn)
    justice = next(
        row for row in registry if row["issue_id"] == "JUSTICE_PUBLIC_SAFETY"
    )
    environment = next((row for row in registry if row["issue_id"] == ISSUE_ID), None)
    national_security = next(
        row for row in registry if row["issue_id"] == "NATIONAL_SECURITY_FOREIGN"
    )
    if (
        _registry_identity(justice)
        != write_set["rollback"]["justice_registry_row_unchanged"]
    ):
        raise StoreSafetyError("M12N changed the Justice registry row")
    if (
        _registry_identity(national_security)
        != write_set["rollback"]["national_security_registry_row_unchanged"]
    ):
        raise StoreSafetyError("M12N changed the National Security registry row")
    if (
        environment is None
        or environment["natural_key"] != M12M_ARTIFACT_ID
        or environment["content_sha256"]
        != write_set["publication_registry"]["publication_metadata"][
            "active_artifact_sha256"
        ]
    ):
        raise StoreSafetyError("M12N Environment & Energy registry identity differs")
    ...
    if environment["publication_metadata_jsonb"] != expected_metadata:
        raise StoreSafetyError("M12N exact publication registry metadata differs")
    if (
        len(registry) != 3
        or _baseline_fingerprint_without_m12n(
            conn, batch_key=write_set["deterministic_batch_key"]
        )
        != write_set["preflight_binding"]["state_fingerprint_sha256"]
    ):
        raise StoreSafetyError("M12N changed Justice or unrelated production rows")
    selector = _selector_state(
    ...
    if (
        ...
        raise StoreSafetyError("M12N public selector postcondition differs")
    for existing_issue in ("JUSTICE_PUBLIC_SAFETY", "NATIONAL_SECURITY_FOREIGN"):
        if (
            ns["119"][existing_issue]["tier"] != "reviewed_conclusion"
            or ns["all"][existing_issue]["tier"] != "reviewed_conclusion"
            or ns["118"][existing_issue]["tier"] != "receipts_only"
        ):
            raise StoreSafetyError(f"M12N changed {existing_issue} selector behavior")
    return {
        "counts": _counts(conn),
    ...
    *,
    ...
    inserted = conn.execute(
        ...
        (
            MEMBER_ID,
            ISSUE_ID,
            ids[M12M_ARTIFACT_ID],
            Jsonb(publication_metadata),
        ),
    )
    if inserted.rowcount != 1:
        raise StoreSafetyError("M12N registry insert count differs")
    return {
...
def _rollback(
    ...
    *,
    ...
    if batch is None:
        raise StoreSafetyError("M12N rollback batch is absent")
    batch_id = int(batch["batch_id"])
    ...
    if (
        ...
        raise StoreSafetyError("M12N rollback write counts differ")
    if _counts(conn) != write_set["expected_counts"]["before"]:
        raise StoreSafetyError("M12N rollback counts differ")
    fingerprint = _state_fingerprint(conn)
    if fingerprint != write_set["rollback"]["restore_state_fingerprint_sha256"]:
        raise StoreSafetyError("M12N rollback fingerprint differs")
    return {
        "counts": _counts(conn),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--deployed-commit", default=POST_M12M_MAIN)
...
def main(argv: list[str] | None = None) -> int:
    ...
    db_url = args.database_url or (
        ...
        else os.getenv("M12N_DISPOSABLE_DATABASE_URL")
    )
    if not db_url:
    ...
    if args.mode in {"dry-run", "apply", "rollback"} and (
        ...
        raise StoreSafetyError("exact M12N write-set digest confirmation required")
