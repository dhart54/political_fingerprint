...
OUT=ROOT/"docs/review_packets/current_house_member_metadata_hardening_v1.json"; MD=OUT.with_suffix(".md"); MAN=ROOT/"docs/source_manifests/current_house_member_metadata_sources_v1.json"; PREVIEW_DIR=ROOT/"docs/review_packets/current_house_member_metadata_hardening_v1"
...
def build(a):
    ...
    mp=batch/"retrieval_manifest.json"; mch=sha256_file(mp)
    previews=build_seed_previews(manifest=rm,manifest_checksum=mch,members=members,house_records=house,clerk_events=clerk,statuses=statuses,production_rows=db["member_rows"]); preview_validation=validate_seed_previews(previews,migration_sql=(ROOT/"backend/migrations/0014_house_member_service_and_seat_status.sql").read_text(encoding="utf-8")); preview_meta=[]
    for name,payload in previews.items():
        serialized=json.dumps(payload,indent=2,sort_keys=True)+"\n"; path=batch/name; path.write_text(serialized,encoding="utf-8"); committed_path=PREVIEW_DIR/name; committed_path.parent.mkdir(parents=True,exist_ok=True); committed_path.write_text(serialized,encoding="utf-8"); preview_meta.append({"path":f"docs/review_packets/current_house_member_metadata_hardening_v1/{name}","row_count":len(payload),"size_bytes":path.stat().st_size,"sha256":sha256_file(path)})
    ...
    vacancy_records=[{k:v for k,v in event.items() if k!="raw_source_text"} for event in clerk]
    report={"schema_version":"current_house_member_metadata_hardening_v1","snapshot":{"snapshot_id":sid,"authoritative_retrieval_timestamp":rm["retrieval_completed_at"],"snapshot_age_days":(date.today()-retrieved_on).days,"fresh":True,"manifest_checksum":mch,"normalized_previews":preview_meta,"preview_schema_validation":preview_validation},"source_retrieval":{"mode":mode,"api_key_present":bool(key),"artifact_count":len(rm["artifacts"]),"artifact_provenance_complete":True,"replay_completeness":rm["replay_completeness"],"current_member_false_count":0,"current_list_detail_disagreement_count":0},"summary":summary,"vacancy_records":vacancy_records,"production_reconciliation":reconciliation,"evidence_lineage":{"design":"snapshot-scoped natural evidence keys linked to snapshot artifact paths with cascading foreign keys","member_service_link_count":len(previews["normalized_member_service_evidence_artifacts.json"]),"seat_status_link_count":len(previews["normalized_seat_status_evidence_artifacts.json"]),"duplicate_links_prevented":True,"rollback_cascades_by_snapshot_id":True},"dc_normalization":{"source_district":"98","canonical_district":"00","rule":"dc_census_98_to_house_delegate_00_v1","auto_select_blocked":True},"proposed_schema":{"migration":"backend/migrations/0014_house_member_service_and_seat_status.sql","migration_applied":False,"table_existence":proposed,"snapshot_tables":["house_member_metadata_snapshots","house_member_metadata_snapshot_artifacts"],"evidence_artifact_tables":["house_member_service_evidence_artifacts","house_seat_status_evidence_artifacts"],"rollback_key":"snapshot_id"},"readiness_impact":{"source_to_member_ready_pair_count":len(ready),"source_to_member_ready_candidate_zcta_count":ready_z,"production_auto_select_eligible_count":0},"verification":{"database_read_only":True,"zip_district_mappings":db["zip_district_mappings"],"route_state":repo["route_state"],"feature_flag":repo["feature_flag"]},"safety":{"database_write_occurred":False,"migration_applied":False,"production_auto_select_enabled":False,"runtime_behavior_changed":False}}
    report["snapshot"]["snapshot_age_days"]=(utc_today-retrieved_on).days
    manifest={"schema_version":"current_house_member_metadata_sources_v1","decision":"approved_for_bounded_dry_run_only","api_key_present":bool(key),"snapshot":report["snapshot"],"retrieval_batch":{k:v for k,v in rm.items() if k!="artifacts"},"artifact_count":len(rm["artifacts"]),"artifacts":rm["artifacts"],"normalized_preview_artifacts":preview_meta,"preview_schema_validation":preview_validation,"evidence_lineage":report["evidence_lineage"]}
...
def inspect_proposed_tables(db_url):
    ...
    names=("house_member_metadata_snapshots","house_member_metadata_snapshot_artifacts","house_member_service_evidence","house_seat_status_evidence","house_member_service_evidence_artifacts","house_seat_status_evidence_artifacts")
...
def render(r):
    s=r["summary"]; q=r["production_reconciliation"]; v=r["vacancy_records"]; return "\n".join(["# Current House Member Metadata Hardening V1","",f"- Snapshot: `{r['snapshot']['snapshot_id']}`",f"- Retrieval timestamp: `{r['snapshot']['authoritative_retrieval_timestamp']}`",f"- Fresh: `{r['snapshot']['fresh']}`",f"- Seat universe: `{json.dumps(s['house_seat_universe'],sort_keys=True)}`",f"- Identity confirmed: `{s['identity_confirmed_member_count']}`",f"- Primary source only: `{s['primary_source_only_count']}`",f"- Active/resolved vacancies: `{s['active_vacancies']}` / `{s['resolved_vacancies']}`",f"- Vacancy records: `{json.dumps(v,sort_keys=True)}`",f"- Replay completeness: `{json.dumps(r['source_retrieval']['replay_completeness'],sort_keys=True)}`",f"- Normalized previews: `{json.dumps(r['snapshot']['normalized_previews'],sort_keys=True)}`",f"- Preview schema validation: `{json.dumps(r['snapshot']['preview_schema_validation'],sort_keys=True)}`",f"- Evidence lineage: `{json.dumps(r['evidence_lineage'],sort_keys=True)}`",f"- Vacancy contradictions: `{json.dumps(q['production_current_rows_on_officially_vacant_seats'],sort_keys=True)}`",f"- Ready pairs/ZCTAs: `{r['readiness_impact']['source_to_member_ready_pair_count']}` / `{r['readiness_impact']['source_to_member_ready_candidate_zcta_count']}`","- Production auto-select: `0`","- Migration applied: `False`","- Production/runtime mutation: `False`",""])
