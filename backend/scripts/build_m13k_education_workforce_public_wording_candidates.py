"""Build detached M13K Education & Workforce public-wording candidates."""
...
from scripts.m13k_public_wording_candidate_data import (  # noqa: E402
    build_wording_definitions,
)


POST_M13J_MAIN = "b69dae58112adbf90db31c4037ddfaffe1a09551"
...
M13J_AUTHORITY_ID = "human-synthesis-authority:f000477:education_workforce:119:v1"
M13J_AUTHORITY_FILE_SHA256 = (
    "a048de2bbfca7cec6568177e01bbe8a00aab2fe66ec6fd919c563707794b9ce9"
)
M13J_AUTHORITY_SUBJECT_SHA256 = (
    "0946cc0343a4f639f0801cfbf467c352612cbae0f8d4d3d9498668f5aed178aa"
)
M13J_IMPLEMENTATION_ID = (
    "synthesis-decision-implementation:f000477:education_workforce:119:v1"
)
M13J_IMPLEMENTATION_FILE_SHA256 = (
    "bf4189d585ff279fc295a2378468c686f5c23d1165fe1d1231cdc80f3c00f8be"
)
M13J_IMPLEMENTATION_SUBJECT_SHA256 = (
    "5472c1bb20a598ed973b475dbcae9a6ea6a1ce98b891f9b09ba07491d97a72ed"
)
...
M13J_AUTHORITY_PATH = M13J_ROOT / "human_synthesis_authority.json"
M13J_IMPLEMENTATION_PATH = M13J_ROOT / "synthesis_decision_implementation.json"
...
PACKAGE_ID = "public-wording-candidates:f000477:education_workforce:119:v1"
DECISION_TEMPLATE_ID = (
    "human-public-wording-decision-template:f000477:education_workforce:119:v1"
)
PARITY_ID = "public-wording-candidate-parity:f000477:education_workforce:119:v1"
...
def write_or_check(path: Path, content: str, check: bool) -> None:
    if check:
        if not path.exists() or path.read_text(encoding="utf-8") != content:
            raise ValueError(f"deterministic M13K artifact differs: {path}")
        return
...
def preflight() -> tuple[
    ...
    paths = {
        ...
        M13J_AUTHORITY_PATH: M13J_AUTHORITY_FILE_SHA256,
        M13J_IMPLEMENTATION_PATH: M13J_IMPLEMENTATION_FILE_SHA256,
    }
    ...
    h_authority = load(M13H_AUTHORITY_PATH)
    h_implementation = load(M13H_IMPLEMENTATION_PATH)
    j_authority = load(M13J_AUTHORITY_PATH)
    j_implementation = load(M13J_IMPLEMENTATION_PATH)
    checks = [
        h_authority["artifact_id"] == M13H_AUTHORITY_ID,
        h_authority["authority_subject_sha256"] == M13H_AUTHORITY_SUBJECT_SHA256,
        h_implementation["artifact_id"] == M13H_IMPLEMENTATION_ID,
        h_implementation["implementation_subject_sha256"]
        == M13H_IMPLEMENTATION_SUBJECT_SHA256,
        j_authority["artifact_id"] == M13J_AUTHORITY_ID,
        j_authority["authority_subject_sha256"] == M13J_AUTHORITY_SUBJECT_SHA256,
        j_implementation["artifact_id"] == M13J_IMPLEMENTATION_ID,
        j_implementation["implementation_subject_sha256"]
        == M13J_IMPLEMENTATION_SUBJECT_SHA256,
        len(h_implementation["subject"]["implementation_records"]) == 2,
        len(j_implementation["subject"]["implementation_records"]) == 0,
        j_implementation["subject"]["canonical_internal_synthesis_state"]
        == "human_accepted_no_safe_synthesis",
        h_implementation["subject"]["blocked_actions"] == [],
    ]
    if not all(checks):
        raise ValueError("accepted M13H/M13J identity or accounting differs")
...
def build_package(
    ...
    return compile_public_wording_candidate_package(
        ...
        subject={
            ...
            "source_authority_boundary": "Accepted M13H Behavioral Semantic IR is the sole positive semantic source. Accepted M13J establishes that the two propositions remain standalone with no safe synthesis; it authorizes no synthesis wording surface. Episode and action lineage is traceability only.",
