"""Build exact M12F policy-episode authority and deterministic implementation."""
...
ACCEPTED_M12E_PR = 152
ACCEPTED_M12E_HEAD = "ecf087f0a6c916ef457014a75381198a16f54857"
POST_M12E_MERGE_MAIN = "450a759c5a2d0eaf767e68bc999c7d3ec8e9ca1e"
CANDIDATE_ID = "policy-episode-candidates:f000477:environment_energy:119:v1"
...
DECISION_TEMPLATE_ID = (
    "policy-episode-human-decisions:f000477:environment_energy:119:v1"
)
...
AUTHORITY_ID = "human-policy-episode-authority:f000477:environment_energy:119:v1"
IMPLEMENTATION_ID = (
...
DECISION_TIMESTAMP = "2026-08-16T01:32:00Z"

CANDIDATE_ROOT = (
    ROOT
    / "docs/editorial/full_record_reviews/policy_episode_candidates/f000477_environment_energy_119_v1"
)
...
def preflight() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if not (
        ...
        raise ValueError("accepted M12D/M12E identity differs")
    if any(row["selected_decision"] is not None for row in template["decisions"]):
        raise ValueError("M12E decision template is not empty")
...
def build_authority(
        ...
        seal(
            {
                ...
                "decision_rationale": (
                    "Independent semantic review accepts this bounded singleton "
    ...
    subject = {
        ...
        "candidate_binding": {
            ...
            "accepted_pr": ACCEPTED_M12E_PR,
            "accepted_head": ACCEPTED_M12E_HEAD,
            "post_merge_main": POST_M12E_MERGE_MAIN,
        },
...
def build_implementation(
    ...
    for source in sorted(
        ...
        record = {
            ...
            "record_id": f"policy-episode-decision-implementation:{source['episode_id']}:m12f:v1",
...
def dossier(authority: dict[str, Any], bundle: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# M12F Environment & Energy Policy-Episode Acceptance",
            "",
            "M12F implements the independently accepted M12E semantic decision exactly as written.",
            "",
            "## Mechanical accounting",
            "",
            "- 63 candidate decisions encoded as `accept_candidate_as_written`.",
            "- 63 accepted actions and 63 accepted singleton episodes.",
            "- Zero multi-action, cross-measure, ambiguous, unassigned, blocked, rejected, or replacement episodes.",
            f"- Direction accounting: {dict(sorted(counts.items()))}.",
            "- H.R. 6387 remains a non-directional Not Voting singleton.",
            "- H.R. 471 and H.R. 3898 remain indivisible whole-package singletons.",
            "- The seven accepted M12E contrast reviews remain review evidence only and create no episode authority.",
            "",
    ...
    parity = seal(
        {
            ...
            "artifact_id": "policy-episode-implementation-parity:f000477:environment_energy:119:v1",
            "authority_subject_sha256": authority["authority_subject_sha256"],
