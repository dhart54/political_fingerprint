...
from app.api.precomputed import (
    get_candidate_evidence_response,
    get_supported_zip_responses,
...
@router.get("/race-candidates/{candidate_id}/evidence")
def lookup_candidate_evidence(candidate_id: str) -> dict[str, object]:
    response = get_candidate_evidence_response(candidate_id=candidate_id)
    if response is None:
