...
export function environmentPositionsForScope(scope) {
  ...
  const interpretedSupport = environmentEvidence119.filter((row) => row.governed_receipt_projection.exact_choice_position_effect === "supports_exact_choice").length;
  const interpretedOppose = environmentEvidence119.filter((row) => row.governed_receipt_projection.exact_choice_position_effect === "opposes_exact_choice").length;
  return { scope_metadata: { congresses: [119], requested_congresses: [119], scope_label: scope === "all" ? "All available Congresses" : "119th Congress" }, positions: [{ domain: "ENVIRONMENT_ENERGY", yea_count: yea, nay_count: nay, other_count: environmentEvidence119.length - yea - nay, total_votes: 63, recorded_votes: yea + nay, interpreted_support_count: interpretedSupport, interpreted_oppose_count: interpretedOppose, interpreted_other_count: environmentEvidence119.length - interpretedSupport - interpretedOppose, interpreted_total: 63 }] };
}
