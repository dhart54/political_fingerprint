...
export default function RecordCardFinding({ entry, legislatorId, scope, receiptsVisible }) {
  const finding = entry.sourceFinding;
  const link = (options) => recordCardUrl("/review/record-card", { legislatorId, scope, ...options });
